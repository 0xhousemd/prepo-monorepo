...
describe('=> UniswapV3OracleUintValue', () => {
  ...
  describe('initial state', () => {
    it('sets static oracle from constructor', async () => {
      expect(await uniswapV3OracleUintValue.getOracle()).eq(fakeUniswapV3Oracle.address)
    })

    it('sets base token from constructor', async () => {
      expect(await uniswapV3OracleUintValue.getBaseToken()).eq(USDC_ADDRESS)
    })

    it('sets quote token from constructor', async () => {
      expect(await uniswapV3OracleUintValue.getQuoteToken()).eq(WETH_ADDRESS)
    })
  })
